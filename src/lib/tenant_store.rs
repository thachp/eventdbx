...
pub struct TenantRecord {
    ...
    #[serde(default, alias = "aggregate_quota")]
...
impl TenantAssignmentStore {
    ...
    pub fn set_storage_usage_bytes(&self, tenant: &str, usage: Option<u64>) -> Result<()> {
        let mut record = self.read_record(tenant)?.unwrap_or_default();
        record.storage_usage_bytes = usage;
        self.write_record(tenant, record)
    }
