...
impl ServerClient {
    ...
    pub fn set_tenant_quota(&self, token: &str, tenant: &str, max_storage_mb: u64) -> Result<bool> {
        let request_id = next_request_id();
        send_control_request_blocking(
            &self.connect_addr,
            token,
            ...
            |request| {
                ...
                quota.set_max_megabytes(max_storage_mb);
                Ok(())
            },
            |response| {
                use control_response::payload;
    ...
    pub fn recalc_tenant_aggregates(&self, token: &str, tenant: &str) -> Result<(u64, u64)> {
        let request_id = next_request_id();
        send_control_request_blocking(
            &self.connect_addr,
            token,
            ...
            |response| {
                ...
                {
                    payload::TenantQuotaRecalc(Ok(reply)) => {
                        Ok((reply.get_aggregate_count(), reply.get_storage_bytes()))
                    }
